...
function UserReport() {
  ...
  useEffect(() => {
    if (submissionData) {
      ...
      const newChartData = submissionData.map(assessment => {
        
        const peermarkGot = assessment.assessment.peerQuestions[0]?.totalMarkGot
        
     

        const usermarkGot = assessment.assessment.userQuestions[0]?.totalMarkGot
        

        const expertmarkGot = assessment.assessment.expertQuestions[0]?.totalMarkGot
      ...
      setChartData(newChartData.map(data => ({
        series: [
          {
                ...
                return <>
                  ...
                  <div className={`flex flex-col border-t-[16px]  border-[#695FDC]  gap-6 rounded-lg  shadow-lg m-10 p-10`}>
                    ...
                    <div className='text-2xl font-semibold '>
                     Task Name - <span className='text-richblue-300'> {submissionData[index].assessment.
                        assignmentTopic
                      }</span>
                    </div>
                  {/* progress */}
                        ...
                        <div className='flex gap-2 justify-center items-center'>
                          <div className='w-[80%] bg-richblue-10 h-[9px] rounded-xl'>
                            ...
                            <div
                              ...
                              style={{ width: (usermarkGot[index].usermarkGot && submissionData[index].assessment?.userQuestions[0]?.totalMark !== 0) ? `${(usermarkGot[index].usermarkGot / (submissionData[index].assessment?.userQuestions[0]?.totalMark || 1)) * 100}%` : '0' }}
                            />
                          </div>
                          <div className='text-gray-800'>
                            <span className='text-lg'>{usermarkGot[index].usermarkGot}/</span>
                            <span className='opacity-75 text-sm'>{submissionData[index].assessment?.userQuestions[0]?.totalMark}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='bg-gray-50 w-60 h-16 rounded-lg p-2'>
                        ...
                        <div className='flex gap-2 justify-center items-center'>
                          <div className='w-[80%] bg-richblue-10 h-[9px] rounded-xl'>
                            <div
                              className="bg-[#8075FF] h-[9px] rounded-xl"
                              style={{ width: usermarkGot[index].peermarkGot && submissionData[index].assessment?.peerQuestions[0]?.totalMark !== 0 ? `${(usermarkGot[index].peermarkGot / (submissionData[index].assessment?.peerQuestions[0]?.totalMark || 1)) * 100}%` : '0' }}
                            />
                          </div>
                          <div className='text-gray-800 '>
                            <span className='text-lg'>{usermarkGot[index].peermarkGot}/</span>
                            <span className='opacity-75 text-sm'>{submissionData[index].assessment?.peerQuestions[0]?.totalMark}</span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className='flex gap-2 justify-center items-center'>
                          <div className='w-[80%] bg-richblue-10 h-[9px] rounded-xl'>
                            <div
                              ...
                              style={{ width: (usermarkGot[index].expertmarkGot && submissionData[index].assessment?.expertQuestions[0]?.totalMark !== 0) ? `${(usermarkGot[index].expertmarkGot / (submissionData[index].assessment?.expertQuestions[0]?.totalMark || 1)) * 100}%` : '0' }}
                            />
                          </div>
                          <div className='text-gray-800'>
                            <span className='text-lg'>{usermarkGot[index].expertmarkGot}/</span>
                            <span className='opacity-75 text-sm'>{submissionData[index].assessment?.expertQuestions[0]?.totalMark}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="app overflow-x-scroll  rounded-lg p-2 w-full sm:max-w-[68%]">
                      <div id="chart">
                        <Chart options={options.options} series={data.series} type="bar" height={200} width={700} /></div>
                    </div>
          ...
          <div className='mt-20'>
            <main class="grid min-h-full ">
              <div class="text-center flex flex-col justify-center items-center">
                ...
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-gray-500 sm:text-2xl">No Submission found</h1>
